void main(){
  print('='*70);
  print('EXEMPLO ORIGINAL: VARREDURA COM METODO map()');
  print('='*70);
   ...
   * O metodo .map() transforma cada entrada em uma String no formato:
   * "chave: valor"
   */
  var resultado = 
  ...
  var precoComDesconto = precos.map((chave,valor) {
    var novoValor =  valor * 0.9;
    return MapEntry(chave, novoValor);
  });

  print('Original: $precos');
  print('Com desconto: $precoComDesconto');
  print('-'*70);

  //==============================================================
  print('EXEMPLO 2: CONVERTENDO MAP EM LISTA DE STRING');
  print('='*70);

  var estoque = {'caneta':50, 'lapis':80,'borracha':25};

  // Convertendo cada par chave/valor em uma frase descritiva
  var descricoes = estoque.entries.map(
    (item) => 'Temos ${item.value} unidade de ${item.key}.',
  );

  // .toList() trqansforma o Iterable em uma lista
